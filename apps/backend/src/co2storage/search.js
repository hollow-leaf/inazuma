...
export async function search_ton_asset(address){
    const authType = "pk"
    const ipfsNodeType = "client"
    //const ipfsNodeAddr = "/ip4/127.0.0.1/tcp/5001"
    //const fgApiUrl = "http://localhost:3020"
...
export async function search_assets(addresses){
    let asset_list = []
    addresses.forEach(address=>{
        const obj = new Promise((resolve)=>{
            resolve(search_ton_asset(address))
        })
        asset_list.push(obj);
    })
    let rres = []
    await Promise.all(asset_list).then(res=>{
